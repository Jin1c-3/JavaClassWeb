<template>
  <div class="app-container">
    <el-form ref="form" :rules="rules" :model="form" label-width="120px">
      ...
      <el-row>
        <el-col :sm="8" :xs="24">
          <el-form-item prop="tem" label="体温">
            <el-input v-model="form.tem" />
          </el-form-item>
        </el-col>

        <el-col :sm="8" :xs="24">
          <el-form-item prop="time" label="测量时间">
            <el-date-picker
              v-model="form.time"
              type="datetime"
              placeholder="选择日期时间"
      ...
      <el-form-item label="有无不适" prop="isIll">
        <el-radio :label="true" v-model="form.isIll">身体不适</el-radio>
        <el-radio :label="false" v-model="form.isIll">无症状</el-radio>
      </el-form-item>

      <el-form-item label="症状描述" v-if="form.isIll" prop="des">
...
export default {
  data() {
    ...
    const isTemperatrue = (rule, value, callback) => {
      if (!value) {
        return new Error("请输入您的体温");
      } else {
        value = Number(value); //转换为数字
        if (typeof value === "number" && !isNaN(value)) {
          //判断是否为数字
          if (value <= 34) {
            callback(new Error("您的体温过低！无法插入"));
          } else if (value >= 41) {
            callback(new Error("您的体温过高！无法插入"));
          } else {
            callback();
          }
        } else {
          callback(new Error("请正确输入体温"));
        }
      }
    };
    ...
    return {
      ...
      rules: {
        ...
        isIll: [{ required: true, message: "请选择身体状况", trigger: "blur" }],
        des: [
          { required: true, message: "请说出您的感受或症状", trigger: blur },
